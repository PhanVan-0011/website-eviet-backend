...
class UpdateSliderRequest extends FormRequest
{
    ...
    {
        $id = $this->route('id'); // hoặc $this->slider nếu bạn dùng route model binding

        $sliderExists = Slider::where('id', $id)->exists();
        return [
            'title' => 'sometimes|required|string|max:200',
            'description' => 'nullable|string|max:255',
            'image_url' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
            'link_url' => 'nullable|string|max:255',
            'display_order' => array_merge(
                ['nullable', 'integer', 'min:1'],
                $sliderExists
                    ? [Rule::unique('sliders', 'display_order')->ignore($id)]
                    : [] // Không kiểm tra unique nếu không tồn tại
            ),
            'is_active' => 'boolean',
            'link_type' => [
                'nullable',
