...
class RolesAndPermissionsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($permissions as $permission) {
            Permission::firstOrCreate(['name' => $permission, 'guard_name' => $guardName]); // <-- SỬA LỖI Ở ĐÂY
        }

        // --- TẠO CÁC VAI TRÒ (ROLES) VÀ GÁN QUYỀN ---

        // 1. Vai trò: Nhân viên Hỗ trợ
        $supportRole = Role::firstOrCreate(['name' => 'support-staff', 'guard_name' => $guardName]); // <-- SỬA LỖI Ở ĐÂY
        $supportRole->givePermissionTo(['orders.view']);

        // 2. Vai trò: Biên tập viên
        $editorRole = Role::firstOrCreate(['name' => 'content-editor', 'guard_name' => $guardName]); // <-- SỬA LỖI Ở ĐÂY
        $editorRole->givePermissionTo(['sliders.manage', 'posts.manage']);
        
        // 3. Vai trò: Quản lý Kho/Sản phẩm
        $productManagerRole = Role::firstOrCreate(['name' => 'product-manager', 'guard_name' => $guardName]); // <-- SỬA LỖI Ở ĐÂY
        ...
        $salesManagerRole = Role::firstOrCreate(['name' => 'sales-manager', 'guard_name' => $guardName]); 
